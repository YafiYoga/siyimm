...
<div class="bg-white p-6 rounded-xl shadow-md max-w-6xl mx-auto overflow-auto max-h-[600px] border border-gray-200 font-[Verdana]">
    ...
    <form method="GET" class="mb-6 flex flex-wrap gap-4 items-end">
        ...
        <a href="{{ route('yayasan.nilai_siswa', ['jenjang' => $jenjang]) }}" class="bg-gray-300 text-gray-800 px-4 py-2 rounded shadow hover:bg-gray-400 transition">
            Reset
        </a>

        <a href="{{ route('yayasan.cetak_nilai', ['jenjang' => $jenjang] + request()->query()) }}" 
           target="_blank"
                                                                ...
                                                                <tbody>
                                                                    @foreach($siswaList as $i => $siswa)
                                                                        <tr class="border-b border-emerald-200 hover:bg-emerald-50">
                                                                            <td class="px-3 py-2 border-r border-emerald-200 text-center align-middle">{{ $i + 1 }}</td>
                                                                            <td class="px-3 py-2 border-r border-emerald-200 align-middle">{{ $siswa['nama_siswa'] }}</td>
                                                                            <td class="px-3 py-2 border-r border-emerald-200 text-center align-middle">{{ $siswa['nilai_tugas'] }}</td>
                                                                            <td class="px-3 py-2 border-r border-emerald-200 text-center align-middle">{{ $siswa['nilai_uts'] }}</td>
                                                                            <td class="px-3 py-2 border-r border-emerald-200 text-center align-middle">{{ $siswa['nilai_uas'] }}</td>
                                                                            <td class="px-3 py-2 text-center align-middle font-semibold">
                                                                                {{ number_format(($siswa['nilai_tugas'] + $siswa['nilai_uts'] + $siswa['nilai_uas']) / 3, 2) }}
                                                                            </td>
                                                                        </tr>
                                                                    @endforeach
                                                                </tbody>
